...
.PHONY: help up prod stop down restart restart-container stop-container beauty composer logs bash psql redis test grpcgen
...
grpcgen:
	docker-compose exec app chmod +x /usr/local/bin/protoc-gen-php-grpc
